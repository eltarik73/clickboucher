...
// LOGO COMPONENT
// ─────────────────────────────────────────────────────────────
function KlikGoLogo({ size = "default", light = false }: { size?: "small" | "default" | "large"; light?: boolean }) {
  const sizes = {
  ...
  const subColor = light ? "text-[#A0A0A0]" : "text-[#9C9590]";
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="flex flex-col">
        <span className={`${s.text} font-bold ${textColor} tracking-tight leading-none`}>
          Klik<span className="text-[#DC2626]">&</span>Go
        </span>
        {size !== "small" && (
          <span className={`text-xs ${subColor} tracking-wider`}>by TkS26</span>
        )}
      </div>
    </div>
  );
...
function HeroLogo() {
  return (
    <div className="flex flex-col items-center mb-6">
      ...
      <p className="text-xs text-[#707070] tracking-wider mt-0.5">by TkS26</p>
    </div>
  );
}
...
function ButcherCard({ butcher }: { butcher: typeof BUTCHERS[0] }) {
  ...
  const handleClick = () => { if (butcher.isOpen) router.push(`/boutique/${butcher.id}`); };

  return (
    <div 
      onClick={handleClick}
      className={`group relative bg-white rounded-2xl border overflow-hidden transition-all duration-300
        ${butcher.isOpen ? "border-[#E8E5E1] shadow-sm hover:shadow-lg hover:border-[#D5D0CA] cursor-pointer" : "border-[#E8E5E1] opacity-60"}`}>
      <div className="relative h-36 overflow-hidden bg-[#F5F3F0]">
        ...
        <div className="absolute top-2.5 left-2.5 flex gap-1.5">
          {butcher.isOpen ? (
            <span className="px-2 py-0.5 rounded-md bg-[#16A34A] text-white text-[10px] font-semibold shadow-sm">Ouvert</span>
          ) : (
            <span className="px-2 py-0.5 rounded-md bg-[#6B6560] text-white text-[10px] font-semibold">Ferme</span>
          )}
      ...
      <div className="p-4">
        <h3 className="font-semibold text-[#1A1A1A] text-[15px] leading-tight mb-1.5">{butcher.name}</h3>
        ...
        <button type="button" disabled={!butcher.isOpen} onClick={(e) => { e.stopPropagation(); handleClick(); }}
          className={`w-full mt-4 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200
            ${butcher.isOpen ? "bg-[#1A1A1A] text-white hover:bg-[#333] active:scale-[0.98]" : "bg-[#F0EDEA] text-[#9C9590] cursor-not-allowed"}`}>
    ...
    <div className="bg-white rounded-xl border border-[#E8E5E1] p-3.5 hover:border-[#DC2626]/30 hover:shadow-sm transition-all cursor-pointer group">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-sm font-semibold text-[#1A1A1A] truncate group-hover:text-[#DC2626] transition-colors">{promo.title}</h4>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#0F0F0F] via-[#1A1A1A] to-[#2A2A2A] text-white">
        ...
        <header className="relative z-20 border-b border-white/10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
            <KlikGoLogo size="default" light={true} />
            <div className="flex items-center gap-3">
              {cart.getItemCount() > 0 && (
                <Link href="/panier" className="relative p-2 rounded-xl bg-white/10 hover:bg-white/20 transition-colors">
              ...
              <Link href="/sign-in" className="px-4 py-2 rounded-xl text-sm font-medium text-white bg-white/10 hover:bg-white/20 transition-colors border border-white/20">
                Se connecter
              </Link>
            </div>
          </div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-20 text-center">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight leading-tight mt-4">
            Marre d attendre ?<br />
            <span className="text-[#FCA5A5]">Commandez, recuperez.</span>
          </h1>
          <p className="mt-4 text-lg sm:text-xl text-[#A0A0A0] max-w-2xl mx-auto">
      ...
      <section id="butchers" className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="hidden lg:block w-[280px] shrink-0">
            <div className="sticky top-24">
              <h3 className="text-sm font-semibold text-[#6B6560] uppercase tracking-wider mb-4">Bons plans du jour</h3>
              <div className="space-y-3">{PROMOS.map(promo => (<PromoCard key={promo.id} promo={promo} />))}</div>
            </div>
          </aside>
        </div>
        <div className="lg:hidden mt-10">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
            {PROMOS.map(promo => (<div key={promo.id} className="shrink-0 w-[220px]"><PromoCard promo={promo} /></div>))}
          </div>
        </div>
      </section>

      {/* ─────────────── FOOTER ─────────────── */}
      <footer className="border-t border-[#E8E5E1] py-8 bg-[#FAFAF9]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col items-center gap-3">
          <KlikGoLogo size="small" />
          <p className="text-sm text-[#9C9590]">2026 Klik&Go - Propulse par TkS26</p>
        </div>
      </footer>
    </div>
  );
