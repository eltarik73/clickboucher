...
function KlikGoLogo({ light = false }: { light?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className={`text-xl font-bold ${textColor} tracking-tight`}>Klik&Go</span>
    </div>
  );
}
...
function HeroLogo() {
  return (
    <div className="flex flex-col items-center mb-8">
      ...
      <h2 className="mt-4 text-2xl sm:text-3xl font-bold text-white tracking-tight">Klik&Go</h2>
...
const BUTCHER_IMAGES = [
  ...
  "https://images.pexels.com/photos/618775/pexels-photo-618775.jpeg?auto=compress&cs=tinysrgb&w=600",
];

const BUTCHERS = [
  { id: "1", name: "Boucherie Dupont", slug: "boucherie-dupont", rating: 4.8, reviews: 127, distance: "800m", time: "15-20 min", isOpen: true, isExpress: true, specialty: "Halal", image: BUTCHER_IMAGES[0] },
  { id: "2", name: "Maison de la Viande", slug: "maison-viande", rating: 4.6, reviews: 89, distance: "1.2km", time: "20-25 min", isOpen: true, isExpress: false, specialty: "Charolais", image: BUTCHER_IMAGES[1] },
  { id: "3", name: "L'Artisan Boucher", slug: "artisan-boucher", rating: 4.9, reviews: 203, distance: "1.5km", time: "15-20 min", isOpen: true, isExpress: true, specialty: "Bio", image: BUTCHER_IMAGES[2] },
  { id: "4", name: "Boucherie Saint-Pierre", slug: "saint-pierre", rating: 4.7, reviews: 156, distance: "2.1km", time: "25-30 min", isOpen: true, isExpress: false, specialty: "Agneau", image: BUTCHER_IMAGES[3] },
];
...
function ButcherCard({ butcher }: { butcher: typeof BUTCHERS[0] }) {
  ...
  const handleClick = () => router.push(`/boutique/${butcher.slug}`);

  return (
    <div onClick={handleClick} className="group cursor-pointer">
      <div className="relative h-52 rounded-2xl overflow-hidden mb-3">
        <img src={butcher.image} alt={butcher.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
        ...
        <div className="absolute top-3 right-3">
          <span className="px-2.5 py-1.5 bg-white/95 backdrop-blur-sm text-gray-800 text-xs font-semibold rounded-lg shadow-sm">{butcher.specialty}</span>
        </div>
        <div className="absolute inset-x-4 bottom-4 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
          <button onClick={(e) => { e.stopPropagation(); handleClick(); }} className="w-full py-3 bg-white text-gray-900 font-semibold rounded-xl shadow-lg hover:bg-gray-100 transition-colors">
            J'y vais
          </button>
        </div>
      </div>
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-gray-900 text-base truncate group-hover:text-[#DC2626] transition-colors">{butcher.name}</h3>
      ...
      <section className="relative bg-[#0A0A0A]">
            ...
            <div className="flex items-center gap-3">
              {itemCount > 0 && (
                <Link href="/panier" className="relative p-2.5 bg-white/5 hover:bg-white/10 rounded-xl transition-colors">
                  ...
                  <span className="absolute -top-1 -right-1 w-5 h-5 bg-[#DC2626] text-white text-[10px] font-bold rounded-full flex items-center justify-center">{itemCount}</span>
                </Link>
              )}
              <Link href="/sign-in" className="px-5 py-2.5 bg-white/5 hover:bg-white/10 text-white text-sm font-medium rounded-xl transition-colors border border-white/10">Se connecter</Link>
            </div>
          </div>
        </header>
        <div className="relative z-10 max-w-6xl mx-auto px-5 py-16 sm:py-20 text-center">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white tracking-tight leading-[1.1]">
            Marre d'attendre ?<br /><span className="text-[#DC2626]">Commandez, recuperez.</span>
          </h1>
          <p className="mt-5 text-lg text-[#888] max-w-xl mx-auto">Zero file. Zero stress. <span className="text-white font-medium">100% frais.</span></p>
          <div className="mt-10 flex flex-col sm:flex-row gap-3 justify-center">
            <button onClick={scrollToButchers} className="px-8 py-4 bg-[#DC2626] hover:bg-[#B91C1C] text-white text-base font-semibold rounded-2xl transition-all shadow-lg shadow-red-500/20 active:scale-[0.98]">Voir les boucheries</button>
            <button className="px-8 py-4 bg-white/5 hover:bg-white/10 text-white text-base font-medium rounded-2xl transition-all border border-white/10">Comment ca marche</button>
          </div>
        </div>
      </section>
      ...
      <section id="butchers" className="max-w-6xl mx-auto px-5 py-14">
        <div className="flex items-end justify-between mb-8">
          ...
          <div className="hidden sm:flex items-center gap-2">
            <button className="px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-full">Tous</button>
            <button className="px-4 py-2 bg-gray-100 text-gray-600 text-sm font-medium rounded-full hover:bg-gray-200 transition-colors">Express</button>
            <button className="px-4 py-2 bg-gray-100 text-gray-600 text-sm font-medium rounded-full hover:bg-gray-200 transition-colors">Halal</button>
          </div>
        </div>
        ...
        <div className="mt-14">
          <div className="flex items-center justify-between mb-5">
            ...
            <button className="text-sm text-[#DC2626] font-medium hover:underline">Voir tout</button>
          </div>
