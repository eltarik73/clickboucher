...
export function ProductSheet({ product, cartQty = 0, onAdd, onIncrement, onDecrement, onClose }: Props) {
  ...
  return (
    <>
      ...
      {/* Sheet */}
      <div
        className={`fixed bottom-0 left-0 right-0 z-50 max-h-[85vh] bg-white dark:bg-[#141414] rounded-t-3xl shadow-2xl transition-transform duration-200 ease-out ${visible ? "translate-y-0" : "translate-y-full"}`}
      >
        {/* Pull handle */}
        ...
        <div className="overflow-y-auto max-h-[calc(85vh-80px)] px-5 pb-6">
          {/* Image */}
          <div className="relative aspect-[16/9] rounded-2xl overflow-hidden bg-gray-100 dark:bg-white/5 mt-2 mb-4">
            <Image
              src={imgSrc}
              alt={product.name}
              fill
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {product.origin && (
              <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-lg bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400">
                <MapPin size={12} />
                {getFlag(product.origin)} {product.origin}
              </span>
            )}
            {product.halalOrg && (
              <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400">
                <Award size={12} />
                Halal {product.halalOrg}
              </span>
            )}
            {product.race && (
              <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-lg bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400">
                <Leaf size={12} />
                Race {product.race}
              </span>
            )}
            {product.freshness && product.freshness !== "STANDARD" && (
              <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-lg bg-cyan-50 dark:bg-cyan-900/20 text-cyan-700 dark:text-cyan-400">
        ...
        {/* Bottom action bar */}
        <div className="sticky bottom-0 px-5 py-4 border-t border-gray-100 dark:border-white/5 bg-white dark:bg-[#141414]">
          {!product.inStock ? (
            <div className="w-full py-3 text-center text-sm font-semibold text-gray-400 bg-gray-100 dark:bg-white/5 rounded-xl">
              Indisponible
            </div>
          ) : cartQty > 0 && !isKg ? (
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-0 rounded-xl overflow-hidden border border-[#DC2626]/20 flex-1">
                ...
                <span className="w-16 text-center text-lg font-bold text-gray-900 dark:text-white tabular-nums">
                  {cartQty}
                </span>
                <button
                  onClick={onIncrement}
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3.5 bg-[#DC2626] hover:bg-[#b91c1c] text-white text-sm font-bold rounded-xl transition-colors active:scale-[0.98]"
            >
              <ShoppingBag size={16} />
              {isKg ? "Choisir le poids" : "Ajouter au panier"}
            </button>
          )}
        </div>
      </div>
